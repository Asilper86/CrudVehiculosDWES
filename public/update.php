...
$vehiculos = Vehiculo::getVehiculoById($id_vehiculo);
...
if (isset($_POST['marca'])) {
    ...
    // si hay errores
    if ($errores) {
        header("Location:{$_SERVER['PHP_SELF']}?id=$id_vehiculo");
        die();
    }
...
<body class="min-h-screen bg-gradient-to-br from-violet-300 via-violet-200 to-violet-100 p-6">
    <form action="<?= $_SERVER['PHP_SELF'] . "?id=" . $id_vehiculo ?>" method="POST"
        class="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-md space-y-4">

        <h2 class="text-2xl font-semibold text-center mb-4">
            <i class="fa-solid fa-car"></i> Actualizar Vehículo
        </h2>
        ...
        <div>
            ...
            <div class="flex items-center border rounded px-3">
                ...
                <input type="text" name="marca" value="<?= $vehiculos->marca ?>" class="w-full py-2 outline-none"
        ...
        <div>
            ...
            <div class="flex items-center border rounded px-3">
                ...
                <input type="text" name="modelo" value="<?= $vehiculos->modelo ?>" class=w-full py-2 outline-none"
            ...
            <div>
                ...
                <div class="flex flex-row items-center gap-6 ml-1 mt-1">
                    ...
                    <label class="flex items-center space-x-1 cursor-pointer">
                        <input type="radio" name="tipo" value="Coche" checked <?=($vehiculos->tipo == "Coche") ? "checked" : ""?>>
                        <span>Coche</span>
                    </label>

                    <label class="flex items-center space-x-1 cursor-pointer">
                        <input type="radio" name="tipo" value="Moto" <?=($vehiculos->tipo == "Moto") ? "checked" : ""?>>
                        <span>Moto</span>
                    </label>

                    <label class="flex items-center space-x-1 cursor-pointer">
                        <input type="radio" name="tipo" value="Camión" <?=($vehiculos->tipo == "Camión") ? "checked" : ""?>>
                        <span>Camión</span>
                    </label>

                    <label class="flex items-center space-x-1 cursor-pointer">
                        <input type="radio" name="tipo" value="Furgoneta" <?=($vehiculos->tipo == "Furgoneta") ? "checked" : ""?>>
                        <span>Furgoneta</span>
                    </label>

                    <label class="flex items-center space-x-1 cursor-pointer">
                        <input type="radio" name="tipo" value="Otro" <?=($vehiculos->tipo == "Otro") ? "checked" : ""?>>
                        <span>Otro</span>
                    </label>
        ...
        <div>
            ...
            <div class="flex items-center border rounded px-3">
                ...
                <input type="number" value="<?=$vehiculos->precio?>" name="precio" step="0.01" min="0" max="999999.99" class="w-full py-2 outline-none"
        ...
        <div>
            ...
            <div class="flex items-start border rounded px-3">
                ...
                <textarea name="descripcion" class="w-full py-2 outline-none" rows="3"
                    placeholder="Descripción del vehículo"><?=$vehiculos->descripcion?></textarea>
            </div>
        ...
        <button type="submit" class="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition">
            <i class="fa-solid fa-edit mr-1"></i> Editar Vehículo
        </button>
    </form>
